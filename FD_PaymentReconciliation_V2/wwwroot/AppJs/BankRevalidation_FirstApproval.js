$(document).ready(function () {
    if (deP_NO == "" || deP_NO == null) {
        ExtendedAjaxCall('/BankRevalidationRequest_FirstApproval/GetdeP_NO', null, 'POST', function (result) {
    ...
    ExtendedAjaxCall('/BankRevalidationRequest/GetRaiseRequest', deP_NO, 'POST', function (result) {
        ...
        if (result != null) {
          ...
          //  $('#chkStaleStatus').text();
            ...
            $("#Rev_Req_No_ID").text(result.data[0].f_REVLD_REQDTLS_REQ_NO);
...
$('#btnApproval').on('click', function () {
    ...
    var Rev_Req_No = $("#Rev_Req_No_ID").text();
    ...
    if (NewOFAS_TC == "0" || (NewOFAS_TC == null || NewOFAS_TC == "")) {
        ...
        ExtendedAjaxCall('/BankRevalidationRequest_FirstApproval/btnApproval_Click', objBO1, 'POST', function (result) {
            
            if (result != null) {
                var Response = result.data;
                if (Response=="Record Saved Succesfully") {
                    $('#lblErrormsg').css('color', '#047904').text("First Approval :" + Response);
                    $('#dvModalMsg').html(Response);
                    $("#ModelMessage").modal('show');

                    $("#OkButton").click(function () {
                        
                        //window.location.href  = WA_FD_PAYMENT_RECO + 'BankRevalidationRequestDashboard_L1';   //FOR UAT
                        //window.location.href = '/BankRevalidationRequestDashboard_L1';   //OR LOCAL
                    ...
                    $('#lblErrormsg').css('color', 'red').text("First Approval :" + Response);
...
$('#btnReject').on('click', function () {
    ...
    if (NewOFAS_TC == "0" || (NewOFAS_TC == null || NewOFAS_TC == "")) {
        ...
        ExtendedAjaxCall('/BankRevalidationRequest_FirstApproval/btnReject_Click', objBO1, 'POST', function (result) {
            
            if (result != null) {
                var Response = result.data;
                if (Response == "Record Saved Succesfully") {
                    $('#lblErrormsg').css('color', '#047904').text("First Rejection : " + Response);
                    $('#dvModalMsg').html(Response);
                    $("#ModelMessage").modal('show');
                    ...
                    $('#lblErrormsg').css('color', 'red').text("First Rejection : " + Response);
                    $('#dvErrorMsg').html(Response);
                    $("#ModelError").modal('show');
                }
            }
...
$("#btnCancel").on('click', function () {
    //window.location.href = WA_FD_PAYMENT_RECO + 'BankRevalidationRequestDashboard_L1';
    var objectURL = InitFunction1();
    window.location.href = objectURL.uploadedData;
});