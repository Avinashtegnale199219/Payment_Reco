...
function bindData() {
    ...
    ExtendedAjaxCall('BankRevalidationRequestDashboard_L2/Get_BankRevalidation_PendingApproval_Dashboard', null, 'GET', function (result) {
        if (result != null) {
            ...
            if (dataResult.status != null && dataResult.status != "0" && dataResult.data != null && dataResult.data.length > 0) {
               ...
               // $('.btnAPRReq').click(function () {
                $(document).on("click", ".btnAPRReq", function (e) {
                    ...
                    ExtendedAjaxCall('BankRevalidationRequestDashboard_L2/SendData', objBO, 'POST', function (result) {
                        if (result.data.P_Dep_NO != "" || result.data.p_War_NO) {
                            window.location.href =  'BankRevalidationRequest_SecondApproval/Index/'; //UAT
                           // window.location.href ='/BankRevalidationRequest_SecondApproval/Index/'; //LOCAL
