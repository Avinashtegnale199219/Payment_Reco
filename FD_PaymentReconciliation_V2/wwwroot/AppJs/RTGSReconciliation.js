$(document).ready(function () {
    $('#lblPageName').text('RTGS Reconciliation');
    $('#lblRecoUpload').text('');
});
    ...
    if (jQuery.inArray(fileExtension[1], extension) != -1) {
        if (file.size > 0) {
            if (window.FormData !== undefined) {
                ...
                $.ajax({
                    url: WA_FD_PAYMENT_RECO +"RTGSReconciliation/Upload_File",
                    type: "POST",
                    data: data,
                    async: false,
                    contentType: false,
...
$('#btnProcess').click(function (e) {
    ...
    if (IsValid()) {
        ...
        var objBO = {
            ...
            Mode: "RTGS_Collection",
            FileName: $('#lblRecoUpload').text(),
            Remarks: $('#txtRemarks').val()
        }

        ExtendedAjaxCall('/RTGSReconciliation/Process_Reco', objBO, 'POST', function (result) {
